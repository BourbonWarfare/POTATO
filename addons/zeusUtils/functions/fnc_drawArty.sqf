...
{
    if !(alive _x && alive gunner _x  && !isPlayer gunner _x) then {continue};
    ...
    if (_missionID == "" || _missionTypeEnum == ARTILLERY_MISSION_STATUS_FREE) then {continue};
    if (ARTILLERY_MISSION_STATUS_WAIT == _status) then {
        private _drawHash = createHashMapFromArray [
            ["@position", [_x, 50]],
            ["@shadow", 4],
            ["@dynamic", true],
            ["@texts", [
