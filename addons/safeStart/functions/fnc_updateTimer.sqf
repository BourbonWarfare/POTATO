...
if (_serverStartTimePV == -1 || isNull (uiNamespace getVariable [QGVAR(timerRscTitle), displayNull])) exitWith { TRACE_3("Bad values: time: %1, display %2", _serverStartTimePV, uiNamespace getVariable [QGVAR(timerRscTitle), displayNull]); };
