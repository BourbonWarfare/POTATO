...
if (_contactDistance > 300 && alive _leader &&
    {side _nearestEnemy != side _leader && side _leader in [east, west, resistance]}) exitWith {
    [{call FUNC(reinforceContactHandler)},
    [_group, false],
...
if (_hasTurret) then {
    [_vic, units _group] call FUNC(reinforceUnloadCargo);
} else {
    [_vic, [objNull]] call FUNC(reinforceUnloadCargo);
};
